...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query(value =
            "SELECT id, name, description, priority, state_id, todo_id FROM tasks WHERE assigned_user_id = ?1 " +
            "UNION " +
            "SELECT id, name, description, priority, state_id, todo_id FROM tasks " +
